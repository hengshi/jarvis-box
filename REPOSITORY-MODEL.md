...
- `production-image.json` 将 release 绑定到其规范源码 commit 和 digest 锁定的生产镜像。
- `create-jarvis` 拥有独立的方法版本。它记录 onboarding 时选择的精确 Jarvis Box release；不将运行时版本复用为自身版本。

## 完成不变量

仅有 GitLab Release 或已部署的生产镜像不构成已完成的公开 release。只有在 tag pipeline 验证每一个公开面后 release 才算完成。GitHub 或下载服务的失败会使 release pipeline 失败，而非留下隐藏的手工后续步骤。
