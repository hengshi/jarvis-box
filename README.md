...
```bash
curl -fsSL https://download.hengshi.com/jarvis-box/install.sh |
  sudo JARVIS_VERSION=0.1.2 bash
```
...
jarvis-box stop
jarvis-box restart
jarvis-box update --check
jarvis-box update --auto
```
...
jarvis-box is a thin orchestration layer. Runtime agents own reasoning, conversation history, native resume, and context compaction. jarvis-box records task state and artifacts; it does not replay old conversation turns into new prompts.
...
Public downloads are published at:
