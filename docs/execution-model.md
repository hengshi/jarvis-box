...
- TaskService 必须先验证每个 repository source 可解析，再在 Task state mutation 中登记初始 Workspace，最后才逐项创建或 clone。TaskService 是 Task Workspace 唯一清理 owner，不依赖 Runtime Foundation reclaimer，也不通过目录扫描猜测归属。Runner 向每个 runtime agent 注入同一 Workspace ownership contract；zero-workspace Task 在 active Run 中取得第一个仓库时必须通过 `jarvis-box workspace create --id primary` 建立 canonical primary，后续增加仓库使用普通 stable id。动态命令同样在 registry mutation 前验证 repository source；`--project` 只有配置 `GITLAB_HOST` 时才能解析，其他仓库必须传 `--remote`。声明 repository 的成功结果必须是真实 Git worktree，只有 project/remote 都为空时才允许空目录。验证通过后才原子登记并创建目录。
...
服务启动和周期恢复只处理已经由进程证据判定为 `recovery-required` 的旧 Run owner：先排除本进程仍在启动的 Run；进程退出已被观察时，本地 Runner 在结果持久化、资源处理和 TaskService finalization 完成前继续持有该 Run 的 completion ownership，恢复器同样必须排除该 Run。这个 ownership 只属于当前服务进程，不持久化；服务重启或 Runner 释放 ownership 后，已有 durable terminalization intent 的 Run 由 terminalization recovery 幂等重放，否则仍无终态且进程证据仍为丢失的 orphan Run 回到 lost-run recovery。Runner 的 completion observation 与 recovery ownership fence 经过同一个精确到 Run 的进程内仲裁，恢复器只有在赢得该仲裁，并在 Task lifecycle 锁下重读状态后证据仍成立时才执行 fence，不向已消失、可能复用的数字 PID 发信号。该动作不创建新 Run；后续工作仍由用户显式 Continue，终结则由 Cancel 完成。
